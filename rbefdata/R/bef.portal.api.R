...
bef.portal.api.dataset_info <- function(id) {
  dataset_xml_to_list <- xmlToList(getURL(dataset_url(dataset_id = id, type = "xml")))
...
bef.portal.api.proposal_info <- function(id) {
  proposal_xml_to_list <- xmlToList(getURL(paperproposal_url(proposal_id = id, type = "xml")))
